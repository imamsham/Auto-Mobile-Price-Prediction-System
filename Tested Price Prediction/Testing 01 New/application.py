...
def index():
    ...
    companies = sorted(df["fuel_type"].unique())
...
def predict():
    company = request.form.get('fuel_type')
    car_model = request.form.get('car_model')
    purchase_year = request.form.get('purchase_year')
    fuel_type = request.form.get('fuel_type')
    kms_driven = request.form.get('kms_driven')

    prediction = model.predict(pd.DataFrame(columns=['name', 'fuel_type', 'year', 'kms_driven', 'fuel_type'], data = np.array([car_model, company, purchase_year, kms_driven, fuel_type]).reshape(1, 5)))
    print(prediction)
